...
- **Never stop the loop** — within a user-started `experiments/train.py` session, keep iterating until the user manually interrupts. Applies only to this experiment loop, never to general coding work.
...
4. If equal or worse → `git reset --hard HEAD~1` (allowed only inside this experiment loop, on `autoresearch/<tag>` branches, against the loop's own just-made commit).
